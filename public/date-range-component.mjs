...
class DateRangeComponent extends HTMLElement {
	static get observedAttributes() {
		return ["label-start", "label-end", "label-today", "label-all", "maxdays"];
	}
	...
	attributeChangedCallback(name, oldValue, newValue) {
		if (name === "maxdays") {
			this.maxdays = parseInt(newValue, 10) || 60;
			// Falls du UI aktualisieren willst:
			this.updateUI();
		}
	}

	init(maxdays) {
		if (this._initialized) return; // verhindert doppelte Initialisierung
		this._initialized = true;
		// Labels initial setzen
		this.labelStart.textContent =
			this.getAttribute("label-start") || "Startdatum:";
	...
	waitForMaxdays() {
		const maxdays = parseInt(this.getAttribute("maxdays"));
		if (maxdays) {
			this.init(maxdays);
			return;
		}

		// Noch nicht gesetzt → in 50ms erneut prüfen
		setTimeout(() => this.waitForMaxdays(), 50);
	}
	...
	validateAndEmit(today, maxDate, maxdays) {
		...
		if (e > maxDate) {
			this.status.textContent =
				"❌ Enddatum muss ≤ heute+" + maxdays + " Tage sein.";
			return;
		}
		if (e < s) {
			this.status.textContent =
